...
- (NSArray*)lgf_PropertyKeys {
    return [[self class] lgf_PropertyKeys];
}
...
- (NSArray *)lgf_PropertiesInfo {
    return [[self class] lgf_PropertiesInfo];
}
/**
...
+ (NSArray *)lgf_PropertiesInfo {
    ...
    for (int i = 0; i < propertyCount; i++)
    {
        [propertieArray addObject:({
            ...
            NSDictionary *dictionary = [self lgf_DictionaryWithProperty:properties[i]];
...
+ (NSArray *)lgf_PropertiesWithCodeFormat {
    ...
    NSArray *properties = [[self class] lgf_PropertiesInfo];
...
- (NSArray *)lgf_MethodListInfo {
    ...
    for (int i = 0; i < count ; i++)
    {
        ...
        for (int index=0; index<argumentsCount; index++) {
            ...
            [arguments addObject:[[self class] lgf_DecodeType:arg]];
        }
        
        NSString *returnTypeString =[[self class] lgf_DecodeType:returnType];
        NSString *encodeString = [[self class] lgf_DecodeType:encoding];
...
- (NSDictionary *)lgf_ProtocolList {
    return [[self class] lgf_ProtocolList];
}
...
+ (NSArray *)lgf_InstanceVariable {
    ...
    NSMutableArray *result = [NSMutableArray array];
    for (int i = 0; i < outCount; i++) {
        NSString *type = [[self class] lgf_DecodeType:ivar_getTypeEncoding(ivars[i])];
...
#pragma mark -- help
+ (NSDictionary *)lgf_DictionaryWithProperty:(objc_property_t)property {
...
+ (NSString *)lgf_DecodeType:(const char *)cString {
    ...
    } else
    {
        if ([[result substringToIndex:1] isEqualToString:@"^"]) {
            result = [NSString stringWithFormat:@"%@ *",
                      [NSString lgf_DecodeType:[[result substringFromIndex:1] cStringUsingEncoding:NSUTF8StringEncoding]]];
        }
    }
    return result;
}
